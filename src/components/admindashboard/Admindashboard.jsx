...
const Admindashboard = ({ route }) => {
  ...
  // --- Wallet modal states (NEW)
  const [showWalletModal, setShowWalletModal] = useState(false)
  const [walletMode, setWalletMode] = useState('add') // 'add' | 'edit'
  ...
  // --- Handlers for wallet modal (NEW)
  const openAddWallets = (userEmail) => {
    setWalletMode('add')
    setWalletData({
      email: userEmail,
      solana: '',
  ...
  const openEditWallets = async (user, userEmail) => {
    // Try to fetch wallet data from backend; if not available, fallback to refer object fields
    setWalletMode('edit')
    setWalletLoading(true)
    setShowWalletModal(true)
    try {
      const req = await fetch(`${route}/api/getWallets`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: userEmail })
      })
      const res = await req.json()
      if (res && (res.solana || res.ethereum || res.tron || res.bitcoin || res.usdt_trc20 || res.usdt_erc20 || res.xrp || res.usdc_erc || res.usdc_trc20)) {
        setWalletData({
          email: userEmail,
          solana: res.solana || '',
          ...
          tron: res.tron || '',
          bitcoin: res.bitcoin || '',
          usdt_trc20: res.usdt_trc20 || '',
          usdt_erc20: res.usdt_erc20 || '',
          xrp: res.xrp || '',
          usdc_erc: res.usdc_erc || '',
          usdc_trc20: res.usdc_trc20 || ''
        })
      } else {
        // fallback to values present on user object (if any)
        setWalletData({
          email: userEmail,
          solana: user.solana || '',
          ...
          usdt_trc20: user.usdt_trc20 || '',
          usdt_erc20: user.usdt_erc20 || '',
          xrp: user.xrp || '',
          usdc_erc: user.usdc_erc || '',
          usdc_trc20: user.usdc_trc20 || ''
        })
      }
    } catch (err) {
      // fallback on error
  ...
  // --- NEW unified handler: one button to Add/Edit wallets
  const openWallets = async (user, userEmail) => {
    setWalletLoading(true)
    setShowWalletModal(true)
    try {
      const req = await fetch(`${route}/api/getWallets`, {
      ...
      const res = await req.json()
      // If backend returns wallets object (status 'ok' and wallets), use it as edit mode
      if (res && res.status === 'ok' && res.wallets && Object.keys(res.wallets).length > 0) {
        setWalletMode('edit')
        setWalletData({
          email: userEmail,
          solana: res.wallets.solana || '',
          ethereum: res.wallets.ethereum || '',
          tron: res.wallets.tron || '',
          bitcoin: res.wallets.bitcoin || '',
          usdt_trc20: res.wallets.usdt_trc20 || '',
          usdt_erc20: res.wallets.usdt_erc20 || '',
          xrp: res.wallets.xrp || '',
          usdc_erc: res.wallets.usdc_erc || '',
          usdc_trc20: res.wallets.usdc_trc20 || ''
        })
      } else {
        // no wallets on backend -> open "add" prefilled from user object if present, else empty
        setWalletMode('add')
        setWalletData({
          email: userEmail,
          solana: user?.solana || '',
      ...
      // on error fallback to user fields and add mode
      setWalletMode('add')
      setWalletData({
        email: userEmail,
        solana: user?.solana || '',
  ...
  // choose correct endpoint for the action
  const endpoint =
    walletMode === 'add'
      ? `${route}/api/addWallets`
      : `${route}/api/updateWallets`;
  ...
  try {
    ...
    if (res && (res.status === 'ok' || res.success || res.message)) {
      Toast.fire({
        icon: 'success',
        title:
          res.message ||
      ...
      Toast.fire({
        icon: 'error',
        title: res.error || 'Could not save wallets',
      });
    }
  } catch (err) {
    ...
    Toast.fire({
      icon: 'error',
      title: 'Network error while saving wallets',
    });
  }
};
               ...
               <motion.div >
                  ...
                  <MdClose className='close-modal-btn' onClick={()=>{setShowUpgradeModal(false)}}/>
                    ...
                    <div className="modal-btn-container">
                      <button class="noselect" onClick={()=>{setShowUpgradeModal(false)}}>
              ...
              showBonusModal && 
               <motion.div >
                  ...
                  <MdClose className='close-modal-btn' onClick={()=>{setShowBonusModal(false)}}/>
                    ...
                    <div className="modal-btn-container">
                      <button class="noselect" onClick={()=>{setShowBonusModal(false)}}>
                        <span class="text">close</span><span class="icon"><svg xmlns="http://www.w3.org/2000/svg"       width="24" height="24" viewBox="0 0 24 24"><path d="M24 20.188l-8.315-8.209 8.2-8.282-3.697-3.697-8.212 8.318-8.31-8.203-3.666 3.666 8.321 8.24-8.206 8.313 3.666 3.666 8.237-8.318 8.285 8.203z"></path></svg></span>
                      </button>
          ...
          >
            <div className="modal-container">
              ...
              <MdClose className='close-modal-btn' onClick={()=>{setDebitModal(false)}}/>
                ...
                <div className="modal-btn-container">
                  <button class="noselect" onClick={()=>{setDebitModal(false)}}>
                    <span class="text">close</span><span class="icon"><svg xmlns="http://www.w3.org/2000/svg"       width="24" height="24" viewBox="0 0 24 24"><path d="M24 20.188l-8.315-8.209 8.2-8.282-3.697-3.697-8.212 8.318-8.31-8.203-3.666 3.666 8.321 8.24-8.206 8.313 3.666 3.666 8.237-8.318 8.285 8.203z"></path></svg></span>
                  </button>
            ...
            {/* ================= Set Preferred Currency Modal ================= */}
              {showCurrencyModal && (
                <div className="currency-modal-overlay">
                  <div className="currency-modal">
                    <h3>Set Preferred Currency</h3>
                    <p>
                      Set preferred currency for <b>{selectedUser?.firstname} {selectedUser?.lastname}</b>
                    </p>
                    ...
                    <div className="modal-actions">
                      <button
                        ...
                        onClick={async () => {
                          try {
                            const req = await fetch(`${route}/api/setPreferredCurrency`, {
                              method: "PATCH",
                              headers: {
                                "Content-Type": "application/json",
                            ...
                            const res = await req.json();
                            if (res.status === "ok") {
                              Swal.fire("Success", "Preferred currency updated!", "success");
                    ...
                    users.map(refer =>
                      <tr key={refer.email}>
                        ...
                        <td>
                          ...
                          className="promo-btn"
                          onClick={() => {
                            setActiveEmail(refer.email); // user row clicked
                        ...
                        {/* NEW: unified Wallets button */}
                        <td>
                          <span
                            className='promo-btn'
                            onClick={() => openWallets(refer, refer.email)}
                          >
                            Wallets
                          </span>
                        </td>
