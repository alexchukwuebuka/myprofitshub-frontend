...
const Userdashboardfundaccount = ({ route }) => {
  ...
  // ✅ Fetch user's wallets dynamically
  useEffect(() => {
    const fetchWallets = async () => {
      try {
        // Assuming user email is stored in localStorage (adjust if using context or token)
        const token = localStorage.getItem('token');
        if (!token) return;

        const res = await fetch(`${route}/api/getMyWallets`, {
          method: 'POST',
        ...
        if (data.status === 'ok' && data.wallets) {
          ...
          const dynamicOptions = [
            { id: 1, min: 1, max: 1000, image: '/btc.png', method: 'BTC', wallet: w.bitcoin },
            { id: 2, min: 1, max: 1000, image: '/etherium.png', method: 'ETH', wallet: w.ethereum },
            { id: 3, min: 1, max: 1000, image: '/tron.png', method: 'USDT (TRC20)', wallet: w.usdt_trc20 },
            { id: 4, min: 1, max: 1000, image: '/usdc-coin.png', method: 'USDC (ERC20)', wallet: w.usdc_erc },
            { id: 5, min: 1, max: 1000, image: '/solana.png', method: 'Solana (SOL)', wallet: w.solana },
            { id: 6, min: 1, max: 1000, image: '/xrp-icon.png', method: 'XRP', wallet: w.xrp },
            { id: 7, min: 1, max: 1000, image: '/tron.png', method: 'TRON', wallet: w.tron },
            { id: 8, min: 1, max: 1000, image: '/usdc-coin.png', method: 'USDC (TRC20)', wallet: w.usdc_trc20 },
            { id: 9, min: 1, max: 1000, image: '/etherium.png', method: 'USDT (ERC20)', wallet: w.usdt_erc20 }
          ].filter(opt => opt.wallet); // remove empty ones

          setDepositOptions(dynamicOptions);
        }
      } catch (err) {
      ...
      {!checkoutPage && (
        <main className="homewrapper">
                ...
                <div className="modal-container">
                  <div className="modal">
                    ...
                    <div className="modal-btn-container">
                      <button className="noselect" onClick={() => setShowModal(false)}>
          ...
          <section className="page-swiper-wrapper">
            ...
            <button className="history-btn" onClick={() => navigate('/deposit')}>
              Deposit history
              <FiArrowRight />
            </button>
          </section>
